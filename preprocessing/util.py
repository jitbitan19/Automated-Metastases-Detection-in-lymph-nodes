...
def get_relative_polygon(polygon, origin: Point, downsample=1):
    ...
    for point in polygon:
        rel_polygon.append(
            Point((point.x - origin.x) / downsample, (point.y - origin.y) / downsample)
        )
...
class TileMap:
    ...
    def __init__(
        self, slide: "Slide", level=None, fill=(20, 180, 8, 80), outline=(20, 180, 8)
    ):
        """
        Parameters
        ----------
        ...
        self._downsample = 2**self._level
        self.tiles = []
        self.image = slide.get_full_slide(self._level)

    def __repr__(self):
        return "{}(slide={!r}, level={!r})".format(
            type(self).__name__, self._slide, self._level
        )
    ...
    def add_tile(self, bounds):
        ...
        rel_poly = get_relative_polygon(poly, Point(0, 0), downsample=self._downsample)

        self.image = draw_polygon(
            self.image, rel_poly, fill=self._fill, outline=self._outline
        )

    def get_dim(self, bounds):
        self.tiles.append(bounds)
        (x, y), (width, height) = bounds
        poly = (
            Point(x, y),
